...
export function BatchDeleteModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-background border border-border rounded-lg shadow-xl w-full max-w-4xl h-[80vh] flex flex-col">
        {/* Fixed Header */}
        ...
        {/* Warning Notice */}
        <div className="px-6 py-4 bg-red-50 dark:bg-red-950/30 border-b border-red-200 dark:border-red-800">
                    ...
                    return allTags.map(tag => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-3 py-1.5 rounded text-xs transition-colors flex items-center gap-1 border ${
                            activeCategory === tag
                              ? 'bg-red-100 text-red-700 dark:bg-red-950/20 dark:text-red-400 border-red-200 dark:border-red-800'
                              : 'text-muted-foreground hover:bg-muted border-transparent'
                          }`}
                  ...
                  {(activeCategory === 'tools' || activeCategory === 'untagged' || (!['agents', 'tools', 'conversations'].includes(activeCategory))) && (
                    <>
                      ...
                      {activeCategory !== 'tools' && activeCategory !== 'untagged' && (
                        <span className="text-sm text-muted-foreground">- {activeCategory}</span>
                      )}
                    </>
                  )}
                  {activeCategory === 'conversations' && (
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              {activeCategory === 'agents' && (
                <div className="space-y-3">
                  {agents.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-8">No agents to delete</p>
                  ) : (
                    agents.map(agent => (
                      <label key={agent.id} className="flex items-start gap-3 p-3 hover:bg-muted/60 rounded border border-border bg-card cursor-pointer">
                        <input
                          type="checkbox"
                ...
                <div className="space-y-3">
                  {(() => {
                    ...
                    if (filteredTools.length === 0) {
                      return <p className="text-sm text-muted-foreground text-center py-8">No tools to delete</p>
                    }

                    return filteredTools.map(tool => (
                      <label key={tool.id} className="flex items-start gap-3 p-3 hover:bg-red-50 dark:hover:bg-red-950/20 rounded border border-border bg-card cursor-pointer">
                        <input
                          type="checkbox"
              ...
              {activeCategory === 'conversations' && (
                <div className="space-y-3">
                  {sessions.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-8">No conversations to delete</p>
                  ) : (
                    sessions.map(session => (
                      <label key={session.id} className="flex items-start gap-3 p-3 hover:bg-red-50 dark:hover:bg-red-950/20 rounded border border-border bg-card cursor-pointer">
                        <input
                          type="checkbox"
                        ...
                        />
                        <div className="flex-1">
                          <div className="font-medium text-foreground">{session.name || 'Untitled Conversation'}</div>
